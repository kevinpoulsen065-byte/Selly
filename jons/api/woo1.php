...
function fetchUrl($url, $cookies, $isPost = false, $postData = [], &$retryCount = 0) {
    ...
    while ($retry > 0) {
        ...
        curl_setopt_array($ch, [
            ...
            CURLOPT_TIMEOUT => 15, // Set timeout to 15 sec
            CURLOPT_HTTPHEADER => [
        ...
        $retry--;
        usleep(500000); // Delay 0.5 seconds before retry
    }
    return false;
}
...
// 1. Get the product page with retry count
$productPage = fetchUrl("https://$domain/?s=&post_type=product", $cookieJar, false, [], $retryCount);

if (!$productPage) {
    echo "#DEAD<font color=white>
    <br><font color=red><small>[PRODUCT PAGE ERROR] [IP:$ip]<br>";
    exit();
}
...
if (!$addToCartId) {
    echo "#DEAD<font color=white>
    <br><font color=red><small>[MISSING PRODUCT ID] [IP:$ip]<br>";
    exit();
}
...
if (!$addToCartResponse) {
    echo "#DEAD<font color=white>
    <br><font color=red><small>[ADD TO CART ERROR] [IP:$ip]<br>";
    exit();
}
...
if (!$checkoutPage) {
    echo "#DEAD<font color=white>
    <br><font color=red><small>[CHECKOUT PAGE ERROR] [IP:$ip]<br>";
    exit();
}
...
if (!isset($matches[1]) || empty($matches[1])) {
    ...
    <br><font color=red><small>[PAYMENT METHOD FAILED TO CAPTURE] [IP:$ip]<br>";
    exit();
}

$paymentMethodsArray = $matches[1];

// Detect CAPTCHA & Cloudflare
